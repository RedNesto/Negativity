...
public class SpeedHackProtocol implements NeedListener {
	...
	public void onPlayerMove(MoveEntityEvent e, @First Player p) {
		...
		if (!np.hasDetectionActive(Cheat.SPEEDHACK))
			return;
		...
		ReportType type = (np.getWarn(Cheat.SPEEDHACK) > 7) ? ReportType.VIOLATION : ReportType.WARNING;
		...
		if (p.isOnGround() && y >= 0.75D) {
			SpongeNegativity.alertMod(type, p, Cheat.SPEEDHACK, Utils.parseInPorcent(y * 100 * 2), proof,
					"Distance Last/New position: " + y + "\n(With same Y)\nPlayer on ground");
		} else if (!p.isOnGround() && y >= 0.85D) {
			mayCancel = SpongeNegativity.alertMod(type, p, Cheat.SPEEDHACK, Utils.parseInPorcent(y * 100 * 2), proof,
					"Distance Last/New position: " + y + "\n(With same Y)\nPlayer jumping");
		}
		if (mayCancel && Cheat.SPEEDHACK.isSetBack())
			e.setCancelled(true);
	}
