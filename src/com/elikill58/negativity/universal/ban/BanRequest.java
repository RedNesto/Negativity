...
public class BanRequest {
	...
	public void execute() {
		...
		NegativityAccount nAccount = ada.getNegativityAccount(this.uuid);
		...
		if (nAccount instanceof NegativityPlayer) {
			...
			np.banEffect();
			np.kickPlayer(reason, new Timestamp(expirationTime).toString().split("\\.", 2)[0], by, def);
		}
	}
	...
	private String getWithReplaceOlder(String s) {
		...
		String gamemode = "?";
		String walkSpeed = "?";
		NegativityAccount nAccount = Adapter.getAdapter().getNegativityAccount(this.uuid);
		if (nAccount instanceof NegativityPlayer) {
			NegativityPlayer np = (NegativityPlayer) nAccount;
