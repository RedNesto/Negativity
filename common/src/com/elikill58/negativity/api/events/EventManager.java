...
public class EventManager {
	...
	public static void registerEvent(Listeners src, @Nullable Boolean useHandle) {
		if (useHandle == null && src instanceof BakedListeners) {
			System.out.println("Found baked listeners");
			BiConsumer<Class<? extends Event>, Listener<?>> registrator = (eventClass, listener) -> {
				registerListener((Listener) listener, (Class) eventClass);
			};
			((BakedListeners) src).bakeListeners(registrator);
			return;
		}
		
		try {
			checkClass(src, Boolean.TRUE.equals(useHandle));
	...
	private static void checkClass(Listeners src, boolean useHandle) throws InstantiationException, IllegalAccessException {
		...
		for(Method m : clazz.getDeclaredMethods()) {
			...
			if(m.getParameterCount() > 1)
				...
				if(isAssignableFrom(paramEvent)) {
					...
					if (useHandle) {
						listener = new HandleBasedListener<>(MethodHandles.lookup().unreflect(m), src);
					} else {
						listener = new ReflectionBasedListener<>(m, src);
					}
	...
	public static void main(String[] args) {
		//registerEvent(new TestListener());
		registerEvent(new TestListener());
